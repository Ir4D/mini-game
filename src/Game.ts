...
export default class Game {
  ...
  set breadcrumbs(value: number) {
    this.state.breadcrumbs = value;
  }
  get time(): number {
  ...
  get enemyTimer(): number {
    return this.state.enemyTimer;
  }
  set enemyTimer(value: number) {
    this.state.enemyTimer = value;
  }
  get collectibleTimer(): number {
    return this.state.collectibleTimer;
  }
  set collectibleTimer(value: number) {
    this.state.collectibleTimer = value;
  }
  get attackReady(): boolean {
    return this.state.attackReady;
  }
  set attackReady(value: boolean) {
    this.state.attackReady = value;
  }
  get attackTimer(): number {
    return this.state.attackTimer;
  }
  set attackTimer(value: number) {
    this.state.attackTimer = value;
  }

  get start(): string {
    return this.state.screen;
  }
  set start(value: GameScreen) {
  ...
  get ground(): number {
    return this.height - this.config.groundLevel;
  }
    ...
    switch (this.state.screen) {
      case "menu":
        this.startButton.setLayout(
          "Start Game",
          this.width * 0.5 - this.startButton.buttonWidth * 0.5,
          this.height * 0.6,
        );
        this.startButton.handlePointerMove(this.pointerX, this.pointerY);
        this.introButton.setLayout(
          "Intro",
          this.width * 0.5 - this.introButton.buttonWidth * 0.5,
          this.height * 0.8,
        );
      ...
      case "intro":
        ...
        return;
      case "pause":
        this.resumeButton.setLayout(
          "Resume",
          this.width * 0.5 - this.resumeButton.buttonWidth * 0.5,
          this.height * 0.4,
        );
      ...
      case "playing":
        ...
        this.pauseButton.setLayout(
          "Pause",
          this.width - this.pauseButton.buttonWidth - 20,
          20,
        );
        ...
        this.collectibles = this.collectibles.filter(
          (collectible) => !collectible.markedForDeletion,
        );
        this.collections = this.collections.filter(
          (collection) => !collection.markedForDeletion,
        );
        this.particles = this.particles.filter(
          (particle) => !particle.markedForDeletion,
        );
        this.attacks = this.attacks.filter(
          (attack) => !attack.markedForDeletion,
        );
